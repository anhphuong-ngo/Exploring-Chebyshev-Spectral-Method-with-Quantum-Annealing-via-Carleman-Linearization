...
def legendre_polynomials(size_per_dim: int):
    return [Legendre.basis(deg) for deg in range(size_per_dim)]

def legendre_derivatives(size_per_dim: int, order: int):
    polys = legendre_polynomials(size_per_dim)
    return [poly.deriv(order) for poly in polys]
...
class TensorProductBasis(Basis):
    ...
    @staticmethod
    def legendre(size_per_dim: int) -> "TensorProductBasis":
        def dphi(k: int, j: int, x: np.ndarray):
            polys = legendre_derivatives(size_per_dim, k)
            return polys[j](x)

        return TensorProductBasis("legendre", size_per_dim, dphi)
...
def basis(name: str, size_per_dim: int, scale: float = 1.0, n_in: int = 1):
    if name == "monomial":
        ...
        return TensorProductBasis.legendre(size_per_dim)
    else:
